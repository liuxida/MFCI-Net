...
class HED(nn.Module):
    ...
    def forward(self, image):
        ...
        image_tensor=image
        # print(type(image_tensor)) <class 'numpy.ndarray'>
        # print(image_tensor.shape)  #(16, 3, 224, 224)

        # Resize the image for faster processing
        batch_size,channels,height, width = image_tensor.shape
        process_images=[]
        for i in range(batch_size):
            image=inv_normalize(image_tensor[i])
            image=image.permute(1,2,0).cpu().numpy()

            # image=image_tensor[i].transpose(1,2,0)
            image = np.uint8((image - image.min()) / (image.max() - image.min()) * 255)  # 确保图像数据范围在 [0, 255]
            ...
            hed_model.setInput(cv2.dnn.blobFromImage(resized, scalefactor=1.0, size=(width, height),
            ...
            # 将 HED 边缘检测结果转换回 PyTorch tensor
            hed_tensor = torch.from_numpy(hed_edges).unsqueeze(0).float() / 255.0  # 添加一个通道维度并归一化

            # hed_tensor = torch.cat([hed_tensor] * 3, dim=0)  # 复制通道以匹配 (3, 224, 224)
            # hed_tensor=hed_tensor.reshape(3,224,224)


            process_images.append(hed_tensor)


        process_images=torch.stack(process_images)
        process_images=process_images.float()

        # # 显示处理结果
        # fig, axes = plt.subplots(4, 4, figsize=(15, 15))
        # for i in range(min(16, process_images.shape[0])):  # 只显示前 16 张图像
        #     hed_image = process_images[i].permute(1, 2, 0).cpu().numpy()
        #
        #     ax = axes[i // 4, i % 4]
        #     ax.imshow(hed_image, cmap='gray')
        #     ax.set_title(f"Image {i + 1}")
        #     ax.axis("off")
        #     # ax.imshow(hed_edges,cmap='gray')
        # plt.tight_layout()
        # plt.show()
